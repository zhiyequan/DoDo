...
package com.bainiaohe.dodo.widgets;
...
public class MaterialMenuDrawable extends Drawable implements Animatable {
    ...
    private final Paint notificationBackgroundPaint = new Paint();
    ...
    private void initPaint(int color) {
        ...
        //TODO set notification paint
        notificationBackgroundPaint.setAntiAlias(true);
        notificationBackgroundPaint.setStyle(Style.FILL_AND_STROKE);
        notificationBackgroundPaint.setColor(Color.RED);


        notificationTextPaint.setAntiAlias(true);
        notificationTextPaint.setColor(color);
     ...
     * @param radius
     */
    private void drawNotification(Canvas canvas, int notificationCount, float centerX, float centerY, float radius) {
        if (notificationCount <= 0)
            return;

        Paint paint = new Paint();
        paint.setColor(Color.RED);
        canvas.drawCircle(centerX, centerY, radius, notificationBackgroundPaint);
//        canvas.drawText("5", stopX + 2, startY, iconPaint);//TODO 显示通知数量
    }
    ...
    private void drawTopLine(Canvas canvas, float ratio) {
        ...
        int alpha = 255;
        //TODO 添加通知
        if (ratio == 0) {//只有当显示burger时才显示通知
            float radius = 10;
            float centerX = width - radius;
            float centerY = topPadding - dip2;
            drawNotification(canvas, 1, centerX, centerY, 10);
//            canvas.drawCircle(stopX + 2, startY, 3, circlePaint);
//            canvas.drawText("5", stopX + 2, startY, iconPaint);
        }
        switch (animationState) {
            case BURGER_ARROW:
                if (isMorphingForward()) {
